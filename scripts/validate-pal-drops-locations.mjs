...
for (const record of records) {
  ...
  for (const drop of record.drops) { assert.ok(typeof drop.name === "string" && drop.name.length, `invalid drop in ${record.palSlug}`); assert.doesNotMatch(drop.name, /[\u4e00-\u9fff]/, `untranslated drop in ${record.palSlug}`); }
