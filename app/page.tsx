...
export default function Home() {
  ...
  return <main id="main-content" className="min-h-screen overflow-hidden bg-canvas text-foreground">
    ...
    <section className="popular-pals-section px-5 py-24 sm:px-8 lg:px-12"><div className="mx-auto max-w-[1360px]"><div className="section-heading-row"><div><p className="eyebrow">Fast Pal Lookup</p><h2 className="section-title mt-3">Popular Pals for Breeding</h2></div><p>Open a focused profile, review current work levels, or search every direct breeding combination for the Pal you want.</p></div><div className="popular-pal-grid">{popularPals.map((pal) => { const details = popularPalDetails.get(pal.slug); return <article key={pal.id}><Link href={`/pals/${pal.slug}`}><PalMark pal={pal} /><div className="popular-pal-card-copy"><span>No. {pal.number}</span><h3>{pal.name}</h3><p>{Object.entries(pal.work).slice(0, 2).map(([key, level]) => <b key={key}>{workGlyphs[key as WorkKey]} {workLabels[key as WorkKey]} {level}</b>)}</p><small>Breeding power {pal.power}</small><div className="popular-pal-card-meta"><span><b>Type</b>{pal.elements.join(" · ")}</span><span><b>Drops</b>{detailNames(details?.drops ?? [])}</span><span><b>Location</b>{details?.locations[0] ? locationLabel(details.locations[0].name) : "Not listed"}</span></div></div></Link><footer><Link href={`/pals/${pal.slug}`}>Profile</Link><Link href={`/breeding-calculator?target=${pal.slug}`}>Combinations</Link></footer></article>; })}</div><Link href="/pals" className="section-cta">Browse 299 standard Pals →</Link></div></section>
