...
import { catalogPals, palCounts, WorkKey, workLabels } from "./lib/game-data";
...
const popularPalDetails = new Map(popularPals.map((pal) => [pal.slug, findPalDropsLocations(pal.slug)]));
...
function detailNames(items: Array<{ name: string }>, empty = "Not listed") {
  const names = [...new Set(items.map((item) => item.name))].slice(0, 2);
  return names.length ? names.join(" · ") : empty;
}

function locationLabel(name: string) {
  const readable = name
    .replaceAll("WorldTreeAura", "World Tree")
    .replaceAll("worldtree", "World Tree")
    .replaceAll("sanctuary", "Wildlife Sanctuary")
    .replaceAll("desertisland", "Desert Island")
    .replaceAll("desert_orange", "Desert")
    .replaceAll("dessert", "Desert")
    .replaceAll("FBOSS", "Boss")
    .replaceAll("_", " ")
    .replace(/\b\d+(?:\s+\d+)*\b/g, "")
    .replace(/\s+/g, " ")
    .trim();
  return readable || "Special encounter";
}
...
export default function Home() {
  ...
  return <main id="main-content" className="min-h-screen overflow-hidden bg-canvas text-foreground">
    ...
    <section className="home-paldex-feature px-5 py-20 sm:px-8 lg:px-12"><div className="mx-auto max-w-[1360px]"><div className="section-heading-row"><div><p className="eyebrow">Pals database</p><h2 className="section-title mt-3">Find the Palworld Pal behind every breeding choice</h2></div><p>Open a focused profile, review current work levels, or search every direct breeding combination for the Pal you want.</p></div><div className="popular-pal-grid home-featured-pal-grid">{popularPals.slice(0, 8).map((pal) => { const details = popularPalDetails.get(pal.slug); const primaryDrop = details?.drops[0]; return <article key={pal.id}><Link href={`/pals/${pal.slug}`}><PalMark pal={pal} /><div className="popular-pal-card-copy"><span>No. {pal.number}</span><h3>{pal.name}</h3><p>{Object.entries(pal.work).slice(0, 2).map(([key, level]) => <b key={key}><WorkSuitabilityIcon work={key as WorkKey} />{workLabels[key as WorkKey]} {level}</b>)}</p><small>Breeding power {pal.power}</small><div className="popular-pal-card-meta"><span><b><span className="popular-pal-meta-icon" aria-hidden="true">◈</span>Type</b><span className="popular-pal-element-list">{pal.elements.map((element) => <span className="popular-pal-element" key={element}><ElementIcon element={element} />{element}</span>)}</span></span><span><b><span className="popular-pal-meta-icon" aria-hidden="true">▣</span>Drops</b><span className="popular-pal-detail-value">{primaryDrop?.icon && <Image src={assetUrl(`/icons/palworld/drops/${primaryDrop.icon}`)} alt="" width={18} height={18} unoptimized />}{detailNames(details?.drops ?? [])}</span></span><span><b><span className="popular-pal-meta-icon" aria-hidden="true">⌖</span>Location</b><span className="popular-pal-detail-value">{details?.locations[0] ? locationLabel(details.locations[0].name) : "Not listed"}</span></span></div></div></Link><footer><Link href={`/pals/${pal.slug}`}>Profile</Link><Link href={`/?mode=target&target=${encodeURIComponent(pal.id)}#breeding-calculator`}>Combinations</Link></footer></article>; })}</div><Link href="/pals" className="section-cta">Browse All Pals →</Link></div></section>

    <section className="popular-pals-section px-5 py-24 sm:px-8 lg:px-12"><div className="mx-auto max-w-[1360px]"><div className="section-heading-row"><div><p className="eyebrow">Fast Pal Lookup</p><h2 className="section-title mt-3">Popular Pals for Breeding</h2></div><p>Open a focused profile, review current work levels, or search every direct breeding combination for the Pal you want.</p></div><div className="popular-pal-grid">{popularPals.map((pal) => { const details = popularPalDetails.get(pal.slug); const primaryDrop = details?.drops[0]; return <article key={pal.id}><Link href={`/pals/${pal.slug}`}><PalMark pal={pal} /><div className="popular-pal-card-copy"><span>No. {pal.number}</span><h3>{pal.name}</h3><p>{Object.entries(pal.work).slice(0, 2).map(([key, level]) => <b key={key}><WorkSuitabilityIcon work={key as WorkKey} />{workLabels[key as WorkKey]} {level}</b>)}</p><small>Breeding power {pal.power}</small><div className="popular-pal-card-meta"><span><b><span className="popular-pal-meta-icon" aria-hidden="true">◈</span>Type</b><span className="popular-pal-element-list">{pal.elements.map((element) => <span className="popular-pal-element" key={element}><ElementIcon element={element} />{element}</span>)}</span></span><span><b><span className="popular-pal-meta-icon" aria-hidden="true">▣</span>Drops</b><span className="popular-pal-detail-value">{primaryDrop?.icon && <Image src={assetUrl(`/icons/palworld/drops/${primaryDrop.icon}`)} alt="" width={18} height={18} unoptimized />}{detailNames(details?.drops ?? [])}</span></span><span><b><span className="popular-pal-meta-icon" aria-hidden="true">⌖</span>Location</b><span className="popular-pal-detail-value">{details?.locations[0] ? locationLabel(details.locations[0].name) : "Not listed"}</span></span></div></div></Link><footer><Link href={`/pals/${pal.slug}`}>Profile</Link><Link href={`/?mode=target&target=${encodeURIComponent(pal.id)}#breeding-calculator`}>Combinations</Link></footer></article>; })}</div><Link href="/pals" className="section-cta">Browse 299 standard Pals →</Link></div></section>
