...
const faqs = [
  ...
  { question: "Is this calculator updated for Palworld 1.0?", answer: "Yes. This Palworld calculator uses the current 300-entry breeding matrix prepared for version 1.0 and loads the result data only when you begin using it." },
...
const standardPalCount = pals.filter((pal) => pal.kind === "pal").length;
const crossoverCreatureCount = pals.filter((pal) => pal.kind === "monster").length;
const totalPalEntryCount = pals.length;
...
export default function Home() {
  ...
  return <main id="main-content" className="min-h-screen overflow-hidden bg-canvas text-foreground">
    ...
    <section className="data-freshness px-5 py-16 sm:px-8 lg:px-12"><div className="mx-auto max-w-[1200px]"><div><span>1.0</span><p><strong>Current data snapshot</strong><small>{standardPalCount} standard Pals · {crossoverCreatureCount} crossover creatures · {totalPalEntryCount} total Pal entries</small></p></div><div><strong>Last data check</strong><span>July 14, 2026</span></div><Link href="/updates">Read the update notes →</Link></div></section>
