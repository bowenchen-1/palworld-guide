...
export default async function PalProfilePage({ params }: Props) {
  ...
  const detailActiveSkills = new Map((detail?.activeSkills ?? []).map((skill) => [skill.name, skill]));
  const englishSkillDescription = (name: string) => detailActiveSkills.get(name)?.description ?? detailActiveSkills.get(name.replace(/^(.*) (Dark|Ice|Ground|Electric|Fire|Neutral|Rainbow|Grass|Water)$/, "$1 ($2)"))?.description;
