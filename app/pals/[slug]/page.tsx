...
export default async function PalProfilePage({ params }: Props) {
  ...
  return <main id="main-content" className="pal-profile-page">
    ...
    <section className="profile-content"><div className="profile-main"><section><p className="database-eyebrow">Verified data</p><h2>{pal.name} 1.0 Paldeck data</h2><div className="profile-stat-strip"><span><small>Paldeck</small><strong>{pal.number}</strong></span><span><small>Breeding power</small><strong>{pal.power}</strong></span><span><small>Work roles</small><strong>{workEntries.length}</strong></span><span><small>Entry type</small><strong>{pal.kind === "pal" ? "Pal" : "Guest"}</strong></span></div><div className="profile-elements" aria-label={`${pal.name} elements`}><span>Elements</span><div>{pal.elements.map((element) => <span className="profile-element" key={element}><ElementIcon element={element} /><b>{element}</b></span>)}</div></div><p>Breeding power is a hidden species value used by many standard Palworld breeding outcomes. It does not describe combat strength, rarity, or catch difficulty. Use it through the calculator instead of treating a lower or higher number as a tier.</p></section>
      <section className="profile-partner-skill"><h2>{pal.name} partner skill</h2>{pal.partnerSkill.name ? <div className="profile-partner-skill-card"><PartnerSkillIcon file={pal.partnerSkill.iconFile} label={pal.partnerSkill.name} /><div><strong>{pal.partnerSkill.name}</strong><p>{pal.partnerSkill.description ?? `The ${pal.partnerSkill.name} partner skill is unique to ${pal.name}. It identifies the special effect this Pal provides when deployed as a partner; check the in-game skill details for its exact activation conditions and values.`}</p></div></div> : <p>No partner skill is recorded for this entry.</p>}</section>
