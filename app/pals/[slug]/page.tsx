...
import { catalogPals, findPal, palCounts, WorkKey, workLabels } from "../../lib/game-data";
...
export default async function PalProfilePage({ params }: Props) {
  ...
  return <main id="main-content" className="pal-profile-page">
    ...
    <section className="profile-hero"><div className="profile-hero-copy"><nav className="profile-breadcrumb" aria-label="Breadcrumb"><Link href="/">Home</Link><span>›</span><Link href="/pals">Pals</Link><span>›</span><b>{pal.name}</b></nav><p className="database-eyebrow">Palworld 1.0 · Paldeck No. {pal.number}</p><h1>{pal.name} Palworld Guide</h1><p>{pal.name} is {pal.kind === "pal" ? "a current Pal" : "a crossover creature"} in version 1.0. This profile lists the verified work-suitability spread and breeding power used by our current Palworld tools.</p><div><Link href={`/?mode=target&target=${encodeURIComponent(pal.id)}`}>Find {pal.name} breeding pairs →</Link><Link href="/pals">Back to Pals</Link></div></div><PalMark pal={pal} showNewBadge /></section>
    <section className="profile-content"><div className="profile-main"><section><p className="database-eyebrow">Verified data</p><h2>{pal.name} 1.0 Paldeck data</h2><div className="profile-stat-strip"><span><small>Paldeck</small><strong>{pal.number}</strong></span><span><small>Breeding power</small><strong>{pal.power}</strong></span><span><small>Work roles</small><strong>{workEntries.length}</strong></span><span><small>Entry type</small><strong>{pal.kind === "pal" ? "Pal" : "Guest"}</strong></span></div><div className="profile-stat-strip profile-stat-strip-details"><span><small>HP</small><strong>{pal.stats.hp ?? "—"}</strong></span><span><small>Defense</small><strong>{pal.stats.defense ?? "—"}</strong></span><span><small>Rarity</small><strong>{pal.rarity ?? "—"}</strong></span><span><small>Sell price</small><strong>{pal.price ?? "—"}</strong></span><span><small>Stamina</small><strong>{pal.stats.stamina ?? "—"}</strong></span><span><small>Food</small><strong>{pal.foodConsumption ?? "—"}</strong></span><span><small>Run speed</small><strong>{pal.movement.run ?? "—"}</strong></span><span><small>Ride speed</small><strong>{pal.movement.rideSprint ?? "—"}</strong></span>{pal.stats.meleeAttack != null && <span><small>Melee attack</small><strong>{pal.stats.meleeAttack}</strong></span>}{pal.stats.rangedAttack != null && <span><small>Ranged attack</small><strong>{pal.stats.rangedAttack}</strong></span>}{pal.stats.support != null && <span><small>Support</small><strong>{pal.stats.support}</strong></span>}{pal.stats.craftSpeed != null && <span><small>Craft speed</small><strong>{pal.stats.craftSpeed}</strong></span>}</div><div className="profile-elements" aria-label={`${pal.name} elements`}><span>Elements</span><div>{pal.elements.map((element) => <span className="profile-element" key={element}><ElementIcon element={element} /><b>{element}</b></span>)}</div></div><p>Breeding power is a hidden species value used by many standard Palworld breeding outcomes. It does not describe combat strength, rarity, or catch difficulty. Use it through the calculator instead of treating a lower or higher number as a tier.</p></section>
