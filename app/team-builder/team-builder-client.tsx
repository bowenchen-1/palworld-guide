    ...
    <div className="team-slots">
      {team.map((id, index) => {
        ...
        return <article key={index} className={`team-slot${pal ? " filled" : ""}${duplicate ? " duplicate" : ""}`}>
          ...
          {pal && <footer><Link href={`/pals/${pal.slug}`}>Profile</Link><Link href={`/?mode=target&target=${encodeURIComponent(pal.id)}#breeding-calculator`}>How to breed</Link><button type="button" onClick={() => remove(index)} aria-label={`Remove ${pal.name} from slot ${index + 1}`}>Remove</button></footer>}
