...
export const metadata: Metadata = {
  ...
  openGraph: { title, description, url: absoluteUrl("/items/hardwood"), siteName: "Palworld Guide", locale: "en_US", type: "website", images: [{ url: absoluteUrl("/items/hardwood/location-map.png"), width: 919, height: 740, alt: "Palworld 1.0 Hardwood location map" }] },
};
...
export default function HardwoodPage() {
  return <main id="main-content" className="hardwood-page min-h-screen bg-canvas text-foreground">
    ...
    <div className="hardwood-wrap">
      ...
      <section id="locations" className="hardwood-section hardwood-location-section scroll-mt-8"><div className="hardwood-section-heading"><p className="hardwood-kicker">01 · Look here first</p><h2>Hardwood locations</h2></div><figure className="hardwood-map-frame"><Image src="/items/hardwood/location-map.png" alt="Schematic Palworld 1.0 Hardwood locations map showing Sakurajima groves and Twilight Dunes" width={919} height={740} priority /><figcaption>Verified route overview · schematic, not an interactive map. Coordinates are approximate and shown only where the source provides them.</figcaption></figure><div className="hardwood-location-grid">{locations.map((location) => <article className="hardwood-location-card" key={location.name}><div className="hardwood-location-top"><span className="hardwood-location-number">{location.name === "Twilight Dunes" ? "A" : "B"}</span><span className="hardwood-tag">{location.label}</span></div><h3>{location.name}</h3><div className="hardwood-fact-row"><span><b>Best for</b>{location.best}</span><span><b>Fast travel</b>{location.travel}</span><span><b>Why go</b>{location.why}</span></div><p className="hardwood-caution-short">⚠ {location.caution}</p></article>)}</div></section>
