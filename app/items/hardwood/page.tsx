...
const sourceMethods = [
  ["Wild rough trees", "Feybreak", "Chop the larger rough/red-leaf trees."],
  ["Logging Site II", "Tech 43", "Passive base production with Lumbering."],
  ["Merchant", "100 gold", "Useful for a small top-up, not a full farm."],
  ["Pal drops", "Bonus only", "Mossanda, Wumpo Botan and Palumba."],
];

const fieldReports = [
  ["Foot of the Volcano", "COMMUNITY REPORT", "Players report the old skeletal trees around the fast-travel point as a reliable Hardwood loop. Treat this as a route lead and test one tree before committing."],
  ["Mount Obsidian · Desiccated Desert · Astral Mountains", "REGIONAL CANDIDATES", "Current location roundups also list dead, spiky, or white biome trees in these regions. Tree models and drop tables can vary by patch, so verify the first chop."],
  ["Feybreak rough-tree clusters", "CROSS-CHECKED", "Multiple 1.0 guides agree on the rough/red-leaf tree model, but none publish a complete per-tree coordinate set."],
];
...
export default function HardwoodPage() {
  return <main id="main-content" className="hardwood-page min-h-screen bg-canvas text-foreground">
    ...
    <div className="hardwood-wrap">
      ...
      <section id="locations" className="hardwood-section hardwood-location-section scroll-mt-8"><div className="hardwood-section-heading"><p className="hardwood-kicker">01 · Feybreak visual route</p><h2>Hardwood tree areas</h2><p>Use the markers as area references, not exact tree coordinates. Current guides confirm rough/red-leaf Hardwood trees on Feybreak, but do not publish a verified per-tree coordinate list.</p></div><figure className="hardwood-map-frame hardwood-map-annotated"><div className="hardwood-map-image"><Image src={assetUrl("/items/hardwood/feybreak-region-map.png")} alt="Feybreak region map with approximate Hardwood rough-tree clusters and route markers" width={1254} height={1254} priority unoptimized /><span className="hardwood-map-marker marker-one"><b>1</b><small>rough-tree<br />cluster</small></span><span className="hardwood-map-marker marker-two"><b>2</b><small>rough-tree<br />cluster</small></span><span className="hardwood-map-marker marker-three"><b>3</b><small>rough-tree<br />cluster</small></span><span className="hardwood-map-marker hardwood-route-marker marker-four"><b>FT</b><small>Scorched<br />Ashland</small></span><span className="hardwood-map-marker hardwood-route-marker marker-five"><b>→</b><small>Fisherman’s<br />Point route</small></span></div><figcaption><strong>Feybreak rough-tree areas · approximate</strong><span>Markers show likely gathering areas on the supplied map; they are not guaranteed individual tree pins. Route markers use confirmed entry references.</span></figcaption></figure><div className="hardwood-map-legend"><span><i className="legend-dot" /> Approximate rough-tree area</span><span><i className="legend-route" /> Route / Fast Travel reference</span><span><i className="legend-island" /> Feybreak island</span></div><div className="hardwood-route-cards"><article><b>ENTRY</b><strong>Fisherman’s Point → Scorched Ashland</strong><span>Fisherman’s Point is the common launch point; Scorched Ashland is the Feybreak waypoint reference at about −814, −823.</span></article><article><b>PREP</b><strong>Level 50+ territory</strong><span>Expect Feybreak faction defenses and anti-air missile turrets before committing to a flying route.</span></article></div><div className="hardwood-location-grid">{locations.map((location) => <article className="hardwood-location-card" key={location.name}><div className="hardwood-location-top"><span className="hardwood-location-number">{location.name === "Twilight Dunes" ? "A" : "B"}</span><span className="hardwood-tag">{location.label}</span></div><h3>{location.name}</h3><div className="hardwood-fact-row"><span><b>Best for</b>{location.best}</span><span><b>Fast travel</b>{location.travel}</span><span><b>Why go</b>{location.why}</span></div><p className="hardwood-caution-short">⚠ {location.caution}</p></article>)}</div></section>

      <section className="hardwood-section hardwood-tree-section" id="recognize"><div className="hardwood-section-heading"><p className="hardwood-kicker">02 · Spot the right tree</p><h2>Tree identification</h2><p>Use the image first. The short notes are only there to confirm what you are seeing.</p></div><div className="hardwood-tree-grid"><figure className="hardwood-tree-card"><Image src={assetUrl("/items/hardwood/twilight-grove.webp")} alt="Sturdy spiky desert trees in Twilight Dunes used for Hardwood" width={1920} height={1080} unoptimized /><figcaption><strong>Twilight Dunes</strong><span>Twisting trunks · spiky leaf crowns · sandy biome</span></figcaption></figure><figure className="hardwood-tree-card"><Image src={assetUrl("/items/hardwood/trees-2.jpg")} alt="Pink Sakura tree being chopped for Hardwood on Sakurajima" width={480} height={270} unoptimized /><figcaption><strong>Sakurajima Sakura</strong><span>Pink canopy · red or white trunk · Hardwood drop shown in-game</span></figcaption></figure></div><div className="hardwood-visual-note">If the tree only gives regular Wood, leave it and look for the biome-specific model.</div></section>
      ...
      <section className="hardwood-section hardwood-source-methods" id="methods"><div className="hardwood-section-heading"><p className="hardwood-kicker">04 · Source comparison</p><h2>Every way to get Hardwood</h2></div><div className="hardwood-source-method-grid">{sourceMethods.map(([name, meta, note]) => <article key={name}><span>{meta}</span><h3>{name}</h3><p>{note}</p></article>)}</div></section>

      <section className="hardwood-section hardwood-field-reports" id="field-reports"><div className="hardwood-section-heading"><p className="hardwood-kicker">05 · Cross-platform checks</p><h2>Extra route leads to test</h2><p>These are useful scouting leads from current community discussions and regional roundups—not guaranteed per-tree pins.</p></div><div className="hardwood-field-report-grid">{fieldReports.map(([name, meta, note]) => <article key={name}><span>{meta}</span><h3>{name}</h3><p>{note}</p></article>)}</div></section>

      <section className="hardwood-section" id="tips"><div className="hardwood-section-heading"><p className="hardwood-kicker">06 · Farm smarter</p><h2>Three field tips</h2></div><div className="hardwood-tip-cards"><article><span>01</span><h3>Carry space</h3><p>Empty your inventory before a long grove run.</p></article><article><span>02</span><h3>Pick the right route</h3><p>Use Twilight Dunes earlier; save Sakurajima for bulk.</p></article><article><span>03</span><h3>Use base production</h3><p>Let a Lumbering Pal handle repeat supply.</p></article></div></section>

      <section className="hardwood-section hardwood-uses-section" id="uses"><div className="hardwood-section-heading"><p className="hardwood-kicker">07 · Spend it</p><h2>What Hardwood unlocks</h2></div><div className="hardwood-use-cards"><article><span>▰</span><h3>High Quality Wooden Board</h3><p>Core mid/late-game material sink.</p></article><article><span>◎</span><h3>High-tier Pal Spheres</h3><p>Used in later capture progression.</p></article><article><span>⌂</span><h3>Weapons & building</h3><p>Appears across 1.0 weapons and Japanese-style structures.</p></article></div></section>
      ...
      <section className="hardwood-section hardwood-sources" id="sources"><div className="hardwood-section-heading"><p className="hardwood-kicker">Research notes</p><h2>Image & fact sources</h2></div><div className="hardwood-source-grid"><a href="https://drawpie.com/blog/palworld-1-0-hardwood-guide/" target="_blank" rel="noreferrer"><strong>Drawpie ↗</strong><span>Location map and 1.0 source comparison.</span></a><a href="https://games.gg/news/palworld-hardwood-locations/" target="_blank" rel="noreferrer"><strong>GAMES.GG ↗</strong><span>Twilight Dunes tree visual and route facts.</span></a><a href="https://www.gamesradar.com/games/rpg/palworld-hardwood/" target="_blank" rel="noreferrer"><strong>GamesRadar+ ↗</strong><span>Regional tree candidates beyond the two primary routes.</span></a><a href="https://www.reddit.com/r/Palworld/comments/1usdsvp/whats_hardwood/" target="_blank" rel="noreferrer"><strong>Reddit field report ↗</strong><span>Community Foot of the Volcano route lead; verify in-game.</span></a><a href="https://www.jeuxvideo.com/news/2093310/bois-robuste-palworld-ou-le-trouver-et-a-quoi-sert-il.htm" target="_blank" rel="noreferrer"><strong>Jeuxvideo.com ↗</strong><span>Sakurajima tree screenshots and Hardwood context.</span></a></div></section>
    </div>
