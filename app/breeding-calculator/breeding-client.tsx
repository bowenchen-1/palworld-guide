...
export default function BreedingClient({ embedded = false }: { embedded?: boolean }) {
  ...
  return <section className={`database-workspace breeding-workspace${embedded ? " home-breeding-workspace" : ""}`} aria-label="Palworld breeding calculator controls">
    ...
    <div className="home-breeding-card home-workbench"><div className="breeding-card-glow" />
      ...
      {mode === "target" && <><div className="home-control-row">{pick("Target Pal", target, "target")}</div>{target && <div className="home-target-actions"><Link href={`/breeding-calculator?target=${target.slug}`} className="home-target-query-link">Find Parent Combinations</Link></div>}<section className="home-results"><header><div><span>Parent combinations</span><strong>{target ? `${pairs.length} found` : "Select a target"}</strong></div>{target && allPairs.length > 0 && resultFilter("Filter by parent", parentFilter, pairCandidates, "parent-filter", () => setParentFilter(undefined))}</header>{list(pairs, "Choose a target to see every direct parent combination.")}</section></>}
