...
export default function BreedingClient({ embedded = false }: { embedded?: boolean }) {
  ...
  const search = useMemo(() => { const n = query.trim().toLowerCase(); return pickerCandidates.filter((pal) => !n || pal.name.toLowerCase().includes(n) || pal.number.toLowerCase().includes(n)).sort(comparePals).slice(0, 60); }, [pickerCandidates, query]);
  ...
  const resultFilter = (label: string, value: string | undefined, candidates: PalData[], next: Extract<Slot, "parent-filter" | "offspring-filter">, clear: () => void) => <div className="home-result-filter"><span>{label}</span><button type="button" className="home-compact-picker" style={{ minHeight: 48, padding: "6px 8px" }} onClick={() => setSlot(next)}><PalMark pal={value ? byId.get(value) : undefined} small /><span><strong>{value ? byId.get(value)?.name : "All Pals"}</strong><em>{value ? `No. ${byId.get(value)?.number}` : `${candidates.length} available`}</em></span><b>{value ? "Change" : "Select"}</b></button>{value && <button type="button" className="home-filter-clear" onClick={clear}>Clear</button>}</div>;
