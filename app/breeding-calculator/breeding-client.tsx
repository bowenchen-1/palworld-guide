...
export default function BreedingClient({ embedded = false }: { embedded?: boolean }) {
  ...
  const pick = (label: string, value: PalData | undefined, next: Exclude<Slot, null>) => <button className="home-compact-picker" onClick={(event) => openPicker(next, event.currentTarget)}><PalMark pal={value} small /><span><small>{label}</small><strong>{value?.name ?? "Search and select a Pal"}</strong>{value && <em>No. {value.number}</em>}</span><b>{value ? "Change" : "Select"}</b></button>;
  const homeParentPick = (label: string, value: PalData | undefined, next: Extract<Slot, "a" | "b">) => <button className={`home-breeding-slot ${value ? "selected" : ""}`} onClick={(event) => openPicker(next, event.currentTarget)}><span className="slot-label">{label}</span><PalMark pal={value} /><strong>{value?.name ?? "Choose a Pal"}</strong><small>{value ? `No. ${value.number}` : "Tap to browse all Pals"}</small><i>{value ? "Change Pal" : "Choose Pal"}</i></button>;
