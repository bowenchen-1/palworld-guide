...
import { availableOffspring, comparePals, findOffspring, findRoutes, pairResults, type BreedingResult } from "./breeding/core";
...
type Slot = "a" | "b" | "target" | "parent" | "start" | "offspring-filter" | "exclude" | null;
...
export default function BreedingClient({ embedded = false }: { embedded?: boolean }) {
  ...
  const pickerCandidates = slot === "offspring-filter" ? offspringCandidates : slot === "exclude" ? pals.filter((pal) => !excluded.includes(pal.id)) : pals;
  ...
  function choose(pal: PalData) { if (slot === "a") setA(pal); if (slot === "b") setB(pal); if (slot === "target") { if (mode === "target") { window.location.assign(`/breeding-calculator?target=${pal.slug}`); return; } setTarget(pal); } if (slot === "parent") { setParent(pal); setOffspringFilter(undefined); } if (slot === "start") setStart(pal); if (slot === "offspring-filter") setOffspringFilter(pal.id); if (slot === "exclude") setExcluded((current) => current.includes(pal.id) ? current : [...current, pal.id]); closePicker(); }
  ...
  return <section className={`database-workspace breeding-workspace${embedded ? " home-breeding-workspace" : ""}`} aria-label="Palworld breeding calculator controls">
    ...
    <div className="home-breeding-card home-workbench"><div className="breeding-card-glow" />
      ...
      {mode === "target" && <div className="home-control-row">{pick("Target Pal", target, "target")}</div>}
