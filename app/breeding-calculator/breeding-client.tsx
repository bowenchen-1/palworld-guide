...
export default function BreedingClient({ embedded = false }: { embedded?: boolean }) {
  ...
  function openPicker(next: Exclude<Slot, null>) { setPickerLimit(PICKER_BATCH_SIZE); setSlot(next); }
  function choose(pal: PalData) { if (slot === "a") setA(pal); if (slot === "b") setB(pal); if (slot === "target") { setTarget(pal); setParentFilter(undefined); } if (slot === "parent") { setParent(pal); setOffspringFilter(undefined); } if (slot === "start") setStart(pal); if (slot === "parent-filter") setParentFilter(pal.id); if (slot === "offspring-filter") setOffspringFilter(pal.id); if (slot === "exclude") setExcluded((current) => current.includes(pal.id) ? current : [...current, pal.id]); setSlot(null); setQuery(""); }
  function copy(text: string) { navigator.clipboard?.writeText(text); }

  const pick = (label: string, value: PalData | undefined, next: Exclude<Slot, null>) => <button className="home-compact-picker" onClick={() => openPicker(next)}><PalMark pal={value} small /><span><small>{label}</small><strong>{value?.name ?? "Search and select a Pal"}</strong>{value && <em>No. {value.number}</em>}</span><b>{value ? "Change" : "Select"}</b></button>;
  const homeParentPick = (label: string, value: PalData | undefined, next: Extract<Slot, "a" | "b">) => <button className={`home-breeding-slot ${value ? "selected" : ""}`} onClick={() => openPicker(next)}><span className="slot-label">{label}</span><PalMark pal={value} /><strong>{value?.name ?? "Choose a Pal"}</strong><small>{value ? `No. ${value.number}` : "Tap to browse all Pals"}</small><i>{value ? "Change Pal" : "Choose Pal"}</i></button>;
  const resultFilter = (label: string, value: string | undefined, candidates: PalData[], next: Extract<Slot, "parent-filter" | "offspring-filter">, clear: () => void) => <div className="home-result-filter"><span>{label}</span><button type="button" className="home-compact-picker" style={{ minHeight: 48, padding: "6px 8px" }} onClick={() => openPicker(next)}><PalMark pal={value ? byId.get(value) : undefined} small /><span><strong>{value ? byId.get(value)?.name : "All Pals"}</strong><em>{value ? `No. ${byId.get(value)?.number}` : `${candidates.length} available`}</em></span><b>{value ? "Change" : "Select"}</b></button>{value && <button type="button" className="home-filter-clear" onClick={clear}>Clear</button>}</div>;
