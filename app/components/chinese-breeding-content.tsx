...
export default function ChineseBreedingContent() {
  ...
  return <>
    ...
    <section className="popular-pals-section px-5 py-24 sm:px-8 lg:px-12"><div className="mx-auto max-w-[1360px]"><div className="section-heading-row"><div><p className="eyebrow">快速查找</p><h2 className="section-title mt-3">常用帕鲁</h2></div><p>打开帕鲁图鉴中的对应记录，确认编号和数据，再继续查找父母组合。</p></div><div className="popular-pal-grid">{popularPals.map((pal) => <article key={pal.id}><Link href={`/zh/pals?ids=${encodeURIComponent(pal.slug)}`}><PalMark pal={pal} /><div><span>编号 {pal.number}</span><h3>{getPalNameZh(pal.name) ?? pal.name}</h3><p>{pal.name}</p><small>配种力 {pal.power}</small></div></Link><footer><Link href={`/zh/pals?ids=${encodeURIComponent(pal.slug)}`}>查看图鉴记录</Link><Link href={`/zh/?mode=target&target=${encodeURIComponent(pal.id)}#breeding-calculator`}>查找父母</Link></footer></article>)}</div><Link href="/zh/pals" className="section-cta">浏览全部 {palCounts.pals} 个帕鲁 →</Link></div></section>

      <section className="tool-lab-section px-5 py-24 sm:px-8 lg:px-12"><div className="mx-auto max-w-[1360px]"><div className="mb-12 grid gap-6 lg:grid-cols-[1fr_.7fr] lg:items-end"><div><p className="eyebrow">配种工具</p><h2 className="section-title mt-3">从选择帕鲁到规划路线</h2></div><p className="max-w-lg text-lg leading-8 text-muted">先查找目标帕鲁，再查看父母组合、子代和路线结果，所有操作都使用同一套配种数据。</p></div><div className="home-update-grid"><Link href="/zh/?mode=parents#breeding-calculator"><span>01</span><h3>双亲 → 子代</h3><p>选择两个父母，查看它们可以配出的子代。</p></Link><Link href="/zh/?mode=target#breeding-calculator"><span>02</span><h3>目标帕鲁 → 父母组合</h3><p>从目标帕鲁反向查找所有直接父母组合。</p></Link><Link href="/zh/?mode=available-route#breeding-calculator"><span>03</span><h3>已有帕鲁 → 目标帕鲁</h3><p>只使用你已经拥有的帕鲁规划路线。</p></Link></div></div></section>
