...
export default function GlobalSearch() {
  ...
  const results = useMemo(() => {
    ...
    const palResults = pals.filter((pal) => matches(`${pal.name} ${pal.number} breeding stats profile`)).map((pal) => ({ href: `/pals/${pal.slug}`, title: pal.name, type: "Pal", detail: `No. ${pal.number}` }));
