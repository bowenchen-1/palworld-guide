...
export default async function ChinesePalProfilePage({ params }: Props) {
  ...
  return <main id="main-content" className="pal-profile-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumb) }} /><div className="profile-nav"><SiteHeader current="/zh/pals" locale="zh" /></div>
    ...
    <section className="profile-content"><div className="profile-main"><section><p className="database-eyebrow">数据详情</p><h2>{nameZh}的帕鲁数据</h2><div className="profile-stat-strip"><span><small>帕鲁编号</small><strong>{pal.number}</strong></span><span><small>配种力</small><strong>{pal.power}</strong></span><span><small>工作类型</small><strong>{workEntries.length}</strong></span><span><small>记录类型</small><strong>{pal.kind === "pal" ? "帕鲁" : "其他生物"}</strong></span></div><div className="profile-stat-strip profile-stat-strip-details"><span><small>生命值</small><strong>{pal.stats.hp ?? "—"}</strong></span><span><small>防御</small><strong>{pal.stats.defense ?? "—"}</strong></span><span><small>稀有度</small><strong>{pal.rarity ?? "—"}</strong></span><span><small>售价</small><strong>{pal.price ?? "—"}</strong></span><span><small>耐力</small><strong>{pal.stats.stamina ?? "—"}</strong></span><span><small>食物消耗</small><strong>{pal.foodConsumption ?? "—"}</strong></span><span><small>运行速度</small><strong>{pal.movement.run ?? "—"}</strong></span><span><small>骑乘速度</small><strong>{pal.movement.rideSprint ?? "—"}</strong></span>{pal.stats.meleeAttack != null && <span><small>近战攻击</small><strong>{pal.stats.meleeAttack}</strong></span>}{pal.stats.rangedAttack != null && <span><small>远程攻击</small><strong>{pal.stats.rangedAttack}</strong></span>}{pal.stats.support != null && <span><small>辅助</small><strong>{pal.stats.support}</strong></span>}{pal.stats.craftSpeed != null && <span><small>制作速度</small><strong>{pal.stats.craftSpeed}</strong></span>}</div><div className="profile-elements" aria-label={`${nameZh}属性`}><span>属性</span><div>{pal.elements.map((element) => <span className="profile-element" key={element}><ElementIcon element={element} /><b>{element}</b></span>)}</div></div><p>配种力是用于计算帕鲁配种结果的物种数据，不代表战斗强度、稀有度或捕捉难度。请通过配种计算器查询实际组合。</p></section>
