...
export default function PaldexPageContent({ initialPage = 1 }: { initialPage?: number }) {
  ...
  return <ToolShell current="/paldex">
    ...
    <section className="database-hero paldex-hero compact-paldex-hero"><div><p className="database-eyebrow">289 current Pal profiles{initialPage > 1 ? ` · page ${initialPage}` : ""}</p><h1>Palworld Paldeck Database{pageSuffix}</h1><p>Search current Pal profiles by name, number, work suitability, and breeding power.</p></div></section>
