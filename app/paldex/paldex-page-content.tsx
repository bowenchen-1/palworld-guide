...
export default function PaldexPageContent({ initialPage = 1 }: { initialPage?: number }) {
  ...
  const schema = {
    ...
    description: `Search ${palCounts.pals} current Palworld 1.0 Pal profiles by number, name, work suitability, and breeding power.`,
  ...
  return <ToolShell current="/paldex" breadcrumb={[{ name: "Home", path: "/" }, { name: "Paldeck", path: pagePath }]}>
    ...
    <section className="database-hero paldex-hero compact-paldex-hero"><div><p className="database-eyebrow">{palCounts.pals} current Pals{initialPage > 1 ? ` · page ${initialPage}` : ""}</p><h1>Palworld Paldeck Database{pageSuffix}</h1><p>Search current Pal profiles by name, number, work suitability, and breeding power.</p></div></section>
