...
export default function PaldexPageContent({ initialPage = 1 }: { initialPage?: number }) {
  ...
  return <ToolShell current="/paldex">
    ...
    <section className="database-hero paldex-hero"><div><p className="database-eyebrow">289 current Pal profiles{initialPage > 1 ? ` · page ${initialPage}` : ""}</p><h1>Palworld Paldeck Database{pageSuffix}</h1><p>Search the version 1.0 Paldeck by name, number, or work suitability, then open a focused profile for each Pal.</p></div><div className="paldex-stack" aria-hidden="true"><span>001</span><span>140</span><span>204</span></div></section>
    ...
    <PaldexClient initialPage={initialPage} />
