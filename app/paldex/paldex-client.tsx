...
export default function PaldexClient({ initialPage: _initialPage = 1 }: { initialPage?: number }) {
  ...
  return <section className="database-workspace paldex-workspace paldex-wide" aria-label="Palworld Paldeck filters and results">
    ...
    {hasFilters && <div className="paldex-chips">{filters.elements.map((value) => <button key={value} onClick={() => update({ elements: toggle(filters.elements, value) })}>{value} ×</button>)}{filters.work.map((value) => <button key={value} onClick={() => update({ work: toggle(filters.work, value) })}>{workLabels[value]} ×</button>)}{filters.workLevel > 0 && <span>Work level {filters.workLevel}+</span>}</div>}
    {visible.length ? <div className="paldex-table-wrap"><table className="paldex-table paldex-complete-table"><thead><tr><th>Pal</th><th>Number</th><th>Element</th><th>Work Suitability</th><th>Partner Skill</th><th>Rarity</th><th>HP</th><th>Breeding Power</th><th>Defense</th><th>Price</th><th>Stamina</th><th>Riding Speed</th><th>Run Speed</th></tr></thead><tbody>{visible.map((pal) => <tr key={pal.id}><td><Link href={`/pals/${pal.slug}`}><PalMark pal={pal} /><span><strong>{pal.name}</strong><small>{pal.kind === "pal" ? "Pal" : "Crossover creature"}</small></span></Link></td><td>{pal.number}</td><td><ElementMarks pal={pal} /></td><td><WorkBadges pal={pal} /></td><td>{pal.partnerSkill.name ? <span className="partner-skill-mark"><PartnerSkillIcon file={pal.partnerSkill.iconFile} label={pal.partnerSkill.name} /><span>{pal.partnerSkill.name}</span></span> : "—"}</td><td>{pal.rarity ?? "—"}</td><td>{pal.stats.hp ?? "—"}</td><td>{pal.power}</td><td>{pal.stats.defense ?? "—"}</td><td>{pal.price ?? "—"}</td><td>{pal.stats.stamina ?? "—"}</td><td>{pal.movement.rideSprint ?? "—"}</td><td>{pal.movement.run ?? "—"}</td></tr>)}</tbody></table></div> : <div className="paldex-empty"><span>⌕</span><h2>No Pals found</h2><p>Try another search or clear one of the filters.</p><button type="button" onClick={clearAll}>Clear all</button></div>}
