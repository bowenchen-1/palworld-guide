...
export default function PaldexClient({ initialPage: _initialPage = 1 }: { initialPage?: number }) {
  ...
  const clearSelection = () => router.push("/pals", { scroll: false });
  ...
  return <section className="database-workspace paldex-workspace paldex-wide" aria-label="Palworld Paldeck filters and results">
    {selectedMode && <div className="paldex-selection-banner" role="status"><strong>{selectedPals.length} Selected Pals</strong><button type="button" onClick={clearSelection}>Clear Selection</button></div>}
