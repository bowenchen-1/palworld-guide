...
export default function PaldexClient() {
  ...
  return <section className="database-workspace paldex-workspace" aria-label="Palworld Paldeck filters and results">
    <div className="database-filters">
      ...
      <label><span>Minimum Work Level</span><select value={minimumLevel} onChange={(event) => setMinimumLevel(Number(event.target.value))}><option value="0">Any level</option>{[1, 2, 3, 4, 5, 6, 7, 8].map((level) => <option key={level} value={level}>Level {level}+</option>)}</select></label>
    </div>
    <div className="paldex-toolbar"><p><strong>{filtered.length}</strong> entries found <span>· current version 1.0 data</span></p><div><label><span>Sort</span><select aria-label="Sort Paldeck" value={sort} onChange={(event) => setSort(event.target.value as SortMode)}><option value="number">Paldeck number</option><option value="name">Name A–Z</option><option value="work">Highest work level</option><option value="power-low">Breeding power: low first</option><option value="power-high">Breeding power: high first</option></select></label><div className="paldex-view-toggle" aria-label="Paldeck view"><button type="button" className={view === "grid" ? "active" : ""} aria-pressed={view === "grid"} onClick={() => setView("grid")}>Grid</button><button type="button" className={view === "list" ? "active" : ""} aria-pressed={view === "list"} onClick={() => setView("list")}>List</button></div><button type="button" className="paldex-reset" onClick={resetFilters}>Reset</button></div></div>
    <div className="paldex-layout"><div>{filtered.length ? <div className={`paldex-grid ${view === "list" ? "list-view" : ""}`}>{filtered.map((pal) => { const strongest = Object.entries(pal.work).sort(([, a], [, b]) => b - a)[0] as [WorkKey, number] | undefined; return <button type="button" aria-pressed={selected.id === pal.id} key={pal.id} onClick={() => setSelected(pal)} className={selected.id === pal.id ? "selected" : ""}><PalMark pal={pal} /><div className="paldex-card-copy"><strong>{pal.name}</strong><p className="mini-work">{Object.entries(pal.work).slice(0, 3).map(([key, level]) => <span key={key}>{workGlyphs[key as WorkKey]} {level}</span>)}</p><small>No. {pal.number} · Breed power {pal.power}</small></div><div className="paldex-card-meta"><span><small>Top role</small><b>{strongest ? `${workGlyphs[strongest[0]]} ${workLabels[strongest[0]]} ${strongest[1]}` : "No work role"}</b></span><span><small>Roles</small><b>{Object.keys(pal.work).length}</b></span><span><small>Power</small><b>{pal.power}</b></span></div></button>; })}</div> : <div className="paldex-empty"><span>⌕</span><h2>No Pals match these filters</h2><p>Try lowering the work level or clearing one of the filters.</p><button type="button" onClick={resetFilters}>Reset filters</button></div>}</div>
